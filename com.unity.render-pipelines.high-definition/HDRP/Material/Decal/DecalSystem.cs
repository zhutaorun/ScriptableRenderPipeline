    ...
    public class DecalSystem
    {
        ...
        private class DecalSet
        {
            ...
            public DecalHandle AddDecal(Matrix4x4 localToWorld, Quaternion rotation, Matrix4x4 sizeOffset, float drawDistance, float fadeScale, Vector4 uvScaleBias, bool affectsTransparency, int materialID)
            {
                ...
                UpdateCachedData(localToWorld, rotation, drawDistance, fadeScale, uvScaleBias, affectsTransparency, decalHandle);
                m_DecalsCount++;
                return decalHandle;
            }
        ...
        DecalHandle AddDecal(Matrix4x4 localToWorld, Quaternion rotation, float drawDistance, float fadeScale, Vector4 uvScaleBias, bool affectsTransparency, Material material)
        {
        ...
        public DecalHandle AddDecal(Vector3 position, Quaternion rotation, Vector3 scale, float drawDistance, float fadeScale, Vector4 uvScaleBias, bool affectsTransparency, Material material)
        {
        ...
        void UpdateCachedData(Matrix4x4 localToWorld, Quaternion rotation, float drawDistance, float fadeScale, Vector4 uvScaleBias, bool affectsTransparency, DecalHandle handle)
        {
            if (!DecalHandle.IsValid(handle))
                return;
            ...
            if (m_DecalSets.TryGetValue(key, out decalSet))
            {
                decalSet.UpdateCachedData(localToWorld, rotation, drawDistance, fadeScale, uvScaleBias, affectsTransparency, handle);
            }
        }
