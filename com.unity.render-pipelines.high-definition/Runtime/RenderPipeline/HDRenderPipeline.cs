        ...
        protected override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            foreach (var camera in cameras)
            {
                ...
                using (new ProfilingSample(cmd, "HDRenderPipeline::Render", CustomSamplerId.HDRenderPipelineRender.GetSampler()))
                {
                    ...
                    if (!camera.TryGetCullingParameters(hdCamera.frameSettings.enableStereo, out cullingParams))
                    {
                        renderContext.Submit();
                        continue;
                    }
                    ...
                    {
                        enableBakeShadowMask = m_LightLoop.PrepareLightsForGPU(cmd, hdCamera, m_ShadowSettings, cullingResults, m_ReflectionProbeCullResults, densityVolumes, m_DebugDisplaySettings);
                    }
        ...
        void RenderDepthPrepass(CullingResults cull, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd)
        {
            ...
            if (hdCamera.frameSettings.enableForwardRenderingOnly)
            {
                using (new ProfilingSample(cmd, "Depth Prepass (forward)", CustomSamplerId.DepthPrepass.GetSampler()))
                {
                    ...
                    RenderOpaqueRenderList(cull, hdCamera, renderContext, cmd, m_DepthOnlyAndDepthForwardOnlyPassNames, PerObjectData.MotionVectors, HDRenderQueue.k_RenderQueue_AllOpaque, excludeMotionVector : excludeMotion);
                }
            }
